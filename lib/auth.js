...
export const login = async () => {
  ...
  const result = await WebBrowser.openAuthSessionAsync(authUrl, appRedirect, { showInRecents: true })
...
export const logout = authToken => {
  return WebBrowser.openAuthSessionAsync(
    `${config.auth.issuer}/session/end?id_token_hint=${authToken}&state=${Constants.linkingUri}`,
    Constants.linkingUri
  ).then(result => {
    if (result.type !== 'success') throw new Error(result)
  })
}
